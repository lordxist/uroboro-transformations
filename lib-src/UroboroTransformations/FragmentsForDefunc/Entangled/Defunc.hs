...
extractPatternMatching fun r@(PTRule l (PQDes l' id pps (PQApp l'' id' pps')) e)
    | any con (pps ++ pps') = do
        pts <- ask
        ...
        lift $ tell $ ruleToHelperFuns fun desTs desRT $ helperFunRule helperFunName r
        return newRule
    | otherwise = return r
extractPatternMatching _ r = return r
