...
&\quad = \textsf{GetPatterns}(q, Q) \cdot \overline{(p_i, \{ p'_i ~ | ~ q'.des(p'_1, ..., p'_k) \in Q \land \textsf{HaveSameDess}(q, q') \})} &
\end{flalign*}
\begin{flalign*}
...
The pre-transformation $align\_patterns$ ensures that, whenever two lhss overlap, one of them is a prefix of the other. $q_\epsilon$ is a prefix of an lhs $q_r$ of the original program; if it overlapped with a $q_{r'}$ taken over unchanged then either (a) $q_{r'}$ would be a prefix of $q_\epsilon$, but then it also would be a prefix of $q_r$, and $q_{r'}$ and $q_r$ overlapped, contrary to assumption, or (b) $q_\epsilon$ would be a prefix of $q_{r'}$, but then, because $q_{r'}$ doesn't have more destructors than $q_r$, either $q_\epsilon = q_{r'}$ or, by the definition of $extract\_des$ and its underlying extraction projection, $extract\_des$ also extracts out of $q_{r'}$, contrary to assumption.

\subsubsection{Pattern extraction}

The extraction $extract\_patterns$ is only ever applied to single-destructor copatterns and transforms them to single-destructor copatterns without any constructors. It is also only ever used in unmixed function definitions. It follows that, if $q_\epsilon$ overlapped with an unchanged equation, the latter has one or more destructors. Since $q_\epsilon$ has only variables as patterns, the unchanged lhs and $q_\epsilon$ would overlap, contrary to assumption.
