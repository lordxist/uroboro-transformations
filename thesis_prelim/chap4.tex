...
The refunctionalization algorithm given below requires the constructor to be at the first position, i.e., it has as its domain the fragment $\mathbf{U}_cc[fdef_{un,r'}]$ of Uroboro defined as follows.

\begin{definition}[Unnested Fragment for the actual \textsf{CoreRefunc}]
The fragment $\mathbf{U}_cc[fdef_{un,r'}]$, for the actual \textsf{CoreRefunc}, is induced by the function definition fragment $fdef_{un,r'}$, defined below using EBNF rules.
\begin{align*}
fdef_{un,r'} &::= eqn_{nr'}^* ~ | ~ eqn_r^* \\
eqn_r &::= fun(x^*).des(y^*)^* = t \\
...
\textbf{Bisimulation.} For \textsf{CoreRefunc}, strong bisimulation holds.
