...
$``\Leftarrow"$: By induction on the structure of $t$. By the derivation of $\langle prg \rangle \vdash_v t$ it is (a) $\langle prg \rangle \vdash_v t'_i$ for all immediate subterms $t'_i$ of $\langle t \rangle$ and (b) $\langle t \rangle \neq^? q'$ for all lhss $q'$ of rules of $\langle prg \rangle$. By the premiss we know that for all lhss $q$ of rules of $prg$, there is a lhs $\langle q \rangle$ of $\langle prg \rangle$; thus it is $t \neq^? q$ for all lhss $q$ of rules of $prg$. By the induction hypothesis, since it is $\langle t_i \rangle = t'_i$ for each immediate subterm $t_i$ of $t$, we know that $prg \vdash_v t_i$. All in all, we have the derivation for $prg \vdash_v t$.
\end{proof}
\end{lemma}
