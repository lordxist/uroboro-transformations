...
We obtain the changed function definition $def'_{fun}$ for $fun$ and the auxiliary function definition in the translated program by applying extraction \textsf{ExtractDes} targeting $q.des(\overline{x^{des}})$, for each $des \in Dess_\tau$, to the original function definition for $fun$. The lhss of $def'_{fun}$ are
\[
...
Let $p$ be the position of $x$ in $q$. We obtain the function definition $def'_{fun}$ for $fun$ in the translated program and the auxiliary function definition by applying extraction \textsf{ExtractCon}(p) targeting $q[x := con(\overline{y^{con}})]$ to the original function definition for $fun$. The lhss of $def'_{fun}$ are
\[
Q \cup \{ \texttt{get}(q[x := con(\overline{y^{con}})]) ~ | ~ con \in Cons_\tau \} = Q \cup \{q\},
\]
where \texttt{get} is the retrieving part of the lens underlying \textsf{ExtractCon}(p). This set of lhss is the same as that in $fun \lhd | ~ Q ~ (q^\tau)$ from the derivation step above.
\end{enumerate}
