...
First, we formally define what it means for a copattern to be \textit{aligned} with another copattern.
...
\begin{lemma}
For any copattern $q$ and set of copatterns $Q$ with $q \rhd Q \searrow \Sigma$ for some $\Sigma$, all elements of $\Sigma(q)$ align with all elements of $Q$.
\end{lemma}

To prove this, we first show a lemma for the sub-algorithm for pattern alignment.

\begin{lemma}
For any two patterns $p_1, p_2$ with $p_1 \rhd p_2 \searrow \Sigma$ for some $\Sigma$, all elements of $\Sigma(p_1)$ align with $p_2$.
...
%\begin{flalign*}
%&\textsf{Align}(x, P^T) &\\
%&\quad= \bigcup \{ con(\overline{\textsf{Align}(\textsf{MapToArg}_i(x_i, P^T))}) ~ | ~ con \in \textrm{Cons}_T \} &&, \text{ if } \exists con . con(\overline{p}) \in P^T &\\
%&\quad= \{x^{\mathit{new}}\}, x^{\mathit{new}} \text{ fresh}, && \text{ otherwise} &\\
%&\textsf{Align}(con(\overline{p}), P^T) &\\
%&\quad= con(\overline{\textsf{Align}(\textsf{MapToArg}_i(p, P^T))}) \span\span
%\end{flalign*}
%\begin{flalign*}
%&\textsf{HaveSameDess}(q.des(\overline{p}), q'.des'(\overline{p'})) &\\
%&\quad = \textsf{HaveSameDess}(q, q') \land des = des' &\\
%&\textsf{HaveSameDess}(fun(\overline{p}), fun(\overline{p'})) &\\
%&\quad = \top &
%\end{flalign*}
%\begin{flalign*}
%&\textsf{GetPatterns}(q.des(\overline{p}), Q) &\\
%&\quad = \textsf{GetPatterns}(q, Q) \cdot \overline{(p_i, \{ p'_i ~ | ~ q'.des(p'_1, ..., p'_k).\overline{des(\overline{p''})} \in Q \land \textsf{HaveSameDess}(q, q') \})} & \\
%&\textsf{GetPatterns}(fun(\overline{p}), Q) &\\
%&\quad = \overline{(p_i, \{ p'_i ~ | ~ fun(p'_1, ..., p'_k).\overline{des(\overline{p''})} \in Q \})}
%\end{flalign*}
%\begin{flalign*}
...
%\end{flalign*}
%\begin{flalign*}
%&\textsf{AlignPatterns}(q, Q) = \{ \textsf{ReplacePatterns}(q, \overrightarrow{p}) ~ | ~  \overrightarrow{p} \in \overrightarrow{\textsf{Align}}(\textsf{GetPatterns}(q, Q)) \} &
%\end{flalign*}
%
%We show a property of \textsf{Align} upon which the intended consequences of $align\_patterns$ depend.
%
%\begin{lemma}
%When $p^1$ and $p^2$ are both contained in a set $P$, $\textsf{Align}(p^1, P)$ and $\textsf{Align}(p^2, P)$ align.
%
%\begin{proof}
%By induction on the structure of $p^1$.
%\end{proof}
%\end{lemma}
%
%The following lemma captures that this transformation actually has its intended consequences.
%
%\begin{lemma}
%For any program $prg$ without overlapping lhss, the following holds for any function definition $def$ of $\langle prg \rangle^{align\_patterns}$: All lhss of $def$ align with all other lhss of $def$.
%
%\begin{proof}
%To show this, it will be shown that, for any set of copatterns $Q$ of the same function $fun$ and any $q_1, q_2 \in Q$, each ${q_1}^a \in \textsf{AlignPatterns}(q_1, Q)$ aligns with each ${q_2}^a \in \textsf{AlignPatterns}(q_2, Q)$. This is done by induction on the structure of $q_1$. In any case it is, for $q \in \{q_1, q_2\}$,
%\[
%\textsf{AlignPatterns}(q, Q) = \{ \textsf{ReplacePatterns}(q, \overrightarrow{p}) ~ | ~  \overrightarrow{p} \in \overrightarrow{\textsf{Align}}(\textsf{GetPatterns}(q, Q)) \}.
%\]
%
%If $q_1 = fun(\overline{p^1})$, it is, for some $\overrightarrow{{p^1}^a} \in \overrightarrow{\textsf{Align}}(\textsf{GetPatterns}(q, Q))$,
%\[
%{q_1}^a = \textsf{ReplacePatterns}(q_1, \overrightarrow{{p^1}^a}) = fun(\overline{{p^1}^a}).
%\]
%And it is
%\[
%\textsf{GetPatterns}(q_1, Q) = \overline{(p^1_i, \{ p'_i ~ | ~ fun(p'_1, ..., p'_k).\overline{des(\overline{p''})} \in Q \})}
%\]
%We proceed by induction on the structure of $q_2$.
%
%If $q_2 = fun(\overline{p^2})$, it is, for some $\overrightarrow{{p^2}^a} \in \overrightarrow{\textsf{Align}}(\textsf{GetPatterns}(q_2, Q))$,
%\[
%{q_2}^a = \textsf{ReplacePatterns}(q_2, \overrightarrow{{p^2}^a}) = fun(\overline{{p^2}^a}).
%\]
%And it is
%\[
%\textsf{GetPatterns}(q_2, Q) = \overline{(p^2_i, \{ p'_i ~ | ~ fun(p'_1, ..., p'_k).\overline{des(\overline{p''})} \in Q \})}
%\]
%By definition, ${q_1}^a$ and ${q_2}^a$ align when ${{p^1}^a}_i$ and  ${{p^2}^a}_i$ align, for every $i \in \{1, ..., n\}$, with $n$ the number of arguments of $fun$. We know that
%\[
%{{p^1}^a}_i = \textsf{Align}(p^1_i, \{ p'_i ~ | ~ fun(p'_1, ..., p'_k).\overline{des(\overline{p''})} \in Q \})
%\]
%and
%\[
%{{p^2}^a}_i = \textsf{Align}(p^2_i, \{ p'_i ~ | ~ fun(p'_1, ..., p'_k).\overline{des(\overline{p''})} \in Q \}).
%\]
%Thus we have that ${{p^1}^a}_i = \textsf{Align}(p^1_i, P)$ and ${{p^2}^a}_i = \textsf{Align}(p^2_i, P)$ for the same set $P$ and, since both $q_1$ and $q_2$ are elements of $Q$, $p^1_i \in P$ and $p^2_i \in P$. By Lemma 4.1.1, it follows that ${{p^1}^a}_i$ and  ${{p^2}^a}_i$ align.
%
%TODO: destructor case for $q_2$ and destructor case for $q_1$
%\end{proof}
%\end{lemma}
