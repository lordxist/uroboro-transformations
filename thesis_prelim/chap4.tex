...
\begin{lemma}
For any program $prg$ without overlapping lhss, no two prefixes of lhss of $\langle prg \rangle^{align\_patterns}$ overlap.
\end{lemma}
...
By Lemma 4.1.1, the pre-transformation $align\_patterns$ ensures that no two prefixes of lhss of the original program overlap. $q_\epsilon$ is a prefix of an lhs $q_r$ of the original program; if it overlapped with a $q_{r'}$ taken over unchanged then $q_{r'}$ (a prefix of itself) and the prefix $q_\epsilon$ of $q_r$ overlapped, contrary to Lemma 4.1.1.
