...
\]
\begin{proof}
We know that $c = q_r[\sigma], \mathcal{E}^{aux}[d] = t_r[\sigma]$ for some equation $r$, and that $b = q_\epsilon[\sigma^{aux}], d = t_\epsilon[\sigma^{aux}]$ for some $\sigma^{aux}$. Since $\mathcal{E}^{aux}[d] = t_r[\sigma]$ and $d = t_\epsilon[\sigma^{aux}] = aux(\overline{x})[\sigma^{aux}]$ and thus $d$ is not a constructor or destructor call, there are two possible cases.
\begin{enumerate}
\item $d$ is a term somewhere within a constructor call in $t_r[\sigma]$ (and, as stated, not a constructor call itself). Thus there is an occurrence of some variable $x$ in $t_r$ such that, for $t^y_r = t_r$ with this occurrence replaced by a fresh (in $t_r$) variable $y$, it is $t^y_r[y \mapsto \mathcal{C}[d]][\sigma] = t_r[\sigma]$ for some context $\mathcal{C}$.
\item $d$ is the term destructed in $t_r[\sigma] = \mathcal{E}^{aux}[d]$, that is, it is $\mathcal{E}^{aux} = [].\overline{des(\overline{x})}$. In this case, since $c \mapsto' \mathcal{E}^{aux}[d]$ and the only equation that $\longrightarrow^{aux}$ uses for contraction, namely $\epsilon$, doesn't have a destructor on its right-hand side, this contraction doesn't use $\epsilon$. All other equations don't have a call to $aux$ on their right-hand side, but $d$ is call to $aux$ and therefore can only have been carried over from $q_r[\sigma]$ via some variable. Thus there also is a variable $x$ in $t_r$ with the same properties as in the first case.
\end{enumerate}
Choose $a = q_r[x \mapsto \mathcal{C}[b]][\sigma]$. The two desired reductions can be derived as follows.
\begin{enumerate}
