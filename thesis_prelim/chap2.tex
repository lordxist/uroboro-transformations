...
In this chapter we formally define the language Uroboro. It is a more or less straightforward generalization of the Data and Codata Fragments of \citet{rendel15automatic}; we therefore follow their presentation. We will also sometimes follow \citet{abel13copatterns}, who also define a language with copatterns, albeit including first-class functions unlike Uroboro.
...
There is one notation that we will presuppose throughout the rest of this work: the Featherweight Java\citep{igarashi01featherweight} ``overbar'' syntax. It is a shorthand for lists of terms of parametrized length, otherwise usually denoted with ellipses. Consider the following example.
\[
fun(t_1, ..., t_n)
\]
...
As is the case with the copattern language of \citet{abel13copatterns}, there are two kinds of types in Uroboro. \textit{Positive types}, which are those defined by data type definitions, and \textit{negative types}, which are those defined by codata type definitions; we get back to this point when we discuss values in \autoref{ssec:val}. The labels ``positive'' and ``negative'' are from \citeauthor{abel13copatterns}, who motivate them by analogy to focusing proofs.

Like \citeauthor{rendel15automatic} do for their fragments, we give a type judgement for terms, and one to judge whether equations are well-typed.  Like them, we let $\Sigma$ be the signatures of a program, that is, its function signatures, codata signatures, and data signatures, as defined above, and we let $\Gamma$ be a typing context, that is, a set of type assignments to variables, and write
\begin{itemize}
...
This section formalizes the notion of value for Uroboro. As is the case with the language of \citet{abel13copatterns}, whether a term is a value depends upon its type. Thus for the following judgement rules a type for every term will be presupposed. Usually, in languages without codata, values are terms which are built up using only constructors. In Uroboro, like in the copattern language of \citeauthor{abel13copatterns}, terms can also be values when they have a negative (codata) type. Unlike in the language of \citeauthor{abel13copatterns}, negatively typed terms aren't always values in Uroboro. Instead, a term is only considered a value if it is in fact irreducible, that is, it doesn't match any lhs of the program. To sum this up, in Uroboro, a term is a value when it is either 
\begin{itemize}
...
This work's evaluation context conservatively extends that for the Data Fragment, since the value judgement conservatively extends the syntactic notion of value for the Data Fragment: For this fragment, the Con rule is identical to the syntax rule \citep[see][Figure 6b]{rendel15automatic}
\[
u, v ::= con(v^*),
\]
