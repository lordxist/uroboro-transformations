...
(SN 2) If $a \in \mathcal{A}, a' \in \mathcal{A}'$ and $a \longrightarrow' a'$ then $\textsf{int}(a) {\longrightarrow}^{\geq 1} \textsf{int}(a')$ or ($\textsf{int}(a) = \textsf{int}(a')$ and $\textsf{m}(a) > \textsf{m}(a')$).
...
\draw[->] (a) -- (c);
\draw[->,dashed] (b) -- (d) node [midway, above, sloped] () {$*$};
...
\draw[->,dashed] (a') -- (b) node [midway, above, sloped] () {$*$};
\draw[->,dashed] (a) -- (c) node [midway, above, sloped] () {$*$};
\draw[->] (b) -- (d);
