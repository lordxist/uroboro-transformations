...
\begin{proof}
The proposition is proven by showing (a) $s {\longrightarrow'}_{\langle prg \rangle}^* t \Rightarrow s \longrightarrow^*_{\langle prg \rangle} \widetilde{t}$ and (b) $s \longrightarrow^*_{\langle prg \rangle} \widetilde{t} \Rightarrow s \longrightarrow^*_{prg} t$. (a) is one direction of (2.2), and from (b) and (2.1) follows the other direction of (2.2).

For both (a) and (b), a conversion function is given that translates sequences of the one reduction relation into corresponding sequences of the other. Then, it is proven that each step of the sequences returned by the function is indeed a reduction step of the relevant reduction, and that the desired equality or equivalence between the end points of the original and the translated sequence holds.

``$\Rightarrow$'': The conversion function $c$ can be defined recursively as follows:
\[
...
TODO: proof of the correctness of the translation (by induction on the length of the sequence)

``$\Leftarrow$'': The unconversion function $uc$ can be defined recursively as follows:
\[
