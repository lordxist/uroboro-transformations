...
\begin{proposition}
Statement (2.2) holds, for every $s,t$ with names declared in $prg$.

\begin{proof}[Sketch of proof]
``$\Rightarrow$'': Each ``minimal'' sequence within $s {\longrightarrow'}^*_{\langle prg \rangle} t$ will be considered, that is, a sequence $s' {\longrightarrow'}^*_{\langle prg \rangle} t'$, for $s', t'$ with names declared in $prg$, that is part of the complete sequence from $s$ to $t$ and where there is no $s''$ with names declared in $prg$ such that $s' {\longrightarrow'}^*_{\langle prg \rangle} s'' {\longrightarrow'}^*_{\langle prg \rangle} t'$. The minimal sequences $s' {\longrightarrow'}^*_{\langle prg \rangle} t'$ are converted to sequences $s' {\longrightarrow'}^*_{\langle prg \rangle} \widetilde{t'} \sim t$. Then, these sequences will be put behind one another, in the order of the original reductions, to get a sequence $s \longrightarrow^*_{\langle prg \rangle} \widetilde{t} \sim t$.

Consider the steps of the sequence $s' {\longrightarrow'}^*_{\langle prg \rangle} t'$. Because of how the value judgement was modified, a reduction step is allowed to ``bypass'' some redexes. Those redexes are exactly the ones which reduce by the $\epsilon$ equation. Furthermore, the first reduction step in the sequence that doesn't use the $\epsilon$ equation necessary leads to $t'$, because the result of this reduction is a term with names declared in $prg$ and $t'$ was defined to be the first such term in the sequence from $s'$ to $t'$.

For the unmodified reduction, it is instead necessary to reduce all these bypassed redexes where they have been bypassed. The $\epsilon$ equation is defined as a call to the auxiliary function that doesn't introduce any new redexes. Because of this, inserting the necessary steps to reduce the bypassed redexes still makes it possible to continue the reduction with the originally following equation afterwards. Since the two sides of $\epsilon$ are equivalent, this sequence with the bypassed steps reinserted leads from $s'$ to a $\widetilde{t'} \sim t'$.

``$\Leftarrow$'': The result of the reduction sequence, $\widetilde{t}$, is $t$ with some instances of the left side of $\epsilon$ replaced with the corresponding right side instances. The reduction sequence cannot depend upon redexes introduced by $\epsilon$, because $\epsilon$ introduces no new redexes. Thus, the desired sequence obtains by removing any step that uses the $\epsilon$ equation and where the resulting call to the auxiliary function isn't removed in further reduction steps.
