...
import UroboroTransformations.FragmentsForDefunc.Entangled.Defunc

import System.IO
import System.Environment

-- |At the moment this is just a test: 
-- parses, typechecks, disentangles the input for defunc., then typechecks and pretty-prints result
-- For this to work, uroboro.cabal needs to be edited to expose Uroboro.PrettyPrint
main :: IO ()
main = do
    input <- getContents
    hPutStrLn stderr "Disentangling (defunc.)... "
