...
For each of the preprocessing steps, a kind of weak bisimulation holds. Each such step is defined as a recursive composition of one of the basic building blocks described in the next section. That weak bisimulation holds for the step is therefore shown by showing it for the lifted extraction function that serves as the building block. This will be done with the help of the propositions below. First, the precise kind of bisimulation is defined by the three statements below. For this, let angular brackets denote the lifted extraction function.
...
As proven in the propositions that follow, the weak bisimulation statements hold for all extraction functions that satisfy a number of properties defined below, simply called the \textit{extraction function properties}.
...
\end{equation*}
where each of the $s^i_j$ is equivalent to exactly one of the $t^0_i$. Thus, it actually is an evaluation context in $prg$, i.e., all of its reduction-relevant subterms left of the hole are irreducible in $prg$. To see this, suppose such a subterm reduced in $prg$, then, by the second extraction function property, the equivalent term among the $t^0_i$ would reduce in $\langle prg \rangle$, contrary to the fact that it is a value in $prg$ because it is to the left of the hole in $\mathcal{E}$.
...
\begin{equation*}
\langle q \rangle^{\sim}_{prg} := \langle q \rangle^{hname}_{prg}(\langle q \rangle^{vars}).
\end{equation*}

 Below, a wrapper for $extract\_des$ is defined that only extracts destructors out of copatterns with a given number of destructors.
\[
    \langle `` q = t ", H \rangle^{extract\_des^s}_{prg} = 
\begin{cases}
    \langle `` q = t ", H \rangle^{extract\_des}_{prg},& \text{if $q$ has $s$ destructors}\\
    \big\langle `` q = t ", H \big\rangle,                   & \text{otherwise}
\end{cases}
\]
