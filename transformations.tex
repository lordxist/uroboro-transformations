...
\end{equation*}
It is $fun(t^1_v, ..., t^n_v) \sim t[\sigma]$, because $q \sim t$. This, in turn, can be shown as follows. For some equation $r$ of $prg$, $`` q = t "$ is contained in $\langle r \rangle^e = \big\langle r', H \big\rangle$. Suppose $`` q = t "$ is in $H$. Then, by the first extraction function property, it is either $q \sim t$, or ($q_r \sim q$ and $t_r \sim t$). But, in the second case, $s^0 \sim fun(t^1_v, ..., t^n_v)$ would already match $q_r$, which contradicts the fact that $s^0$ is a value in $prg$. Now, suppose $r' = `` q = t "$. Then, by the third extraction function property, it is either $q \sim t$, or ($r = `` q = t "$ and $H = \emptyset$). But, in the second case, $s^0 \sim fun(t^1_v, ..., t^n_v)$ would already match $q_r$, which contradicts the fact that $s^0$ is a value in $prg$.
...
\end{equation*}
analogously to Case 1 (regard $t^0$ simply as an initial argument). This result then has to be reduced similarly to Case 2, with the only difference being that a rule with destructor pattern is used instead of one with hole pattern.
