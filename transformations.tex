...
In this section it is shown that the transformations above preserve the semantics of programs in a kind of weak bisimulation. Precisely, for all transformations indicated by angular brackets, it holds for all closed input terms $s$ and all programs $prg$ that:

For all closed terms $t$: $s \longrightarrow^*_{prg} t \implies \langle s \rangle \longrightarrow^*_{\langle prg \rangle} \langle t \rangle$

and

if there is an infinite reduction sequence in $\langle prg \rangle$ starting with s, i.e., it holds for all $s_1$ in the sequence that there exists an $s_2$ in the sequence with $s_1 \longrightarrow_{\langle prg \rangle} s_2$, then:

$\exists t, s_0: s \longrightarrow^*_{\langle prg \rangle} t \land s_0 \longrightarrow^*_{\langle prg \rangle} s \land s_0 \longrightarrow^*_{prg} t$ (2)

The first statement follows from (1) as given below by the transitivity of the reduction relation.

$s \longrightarrow_{prg} t \implies \langle s \rangle \longrightarrow_{\langle prg \rangle}^* \langle t \rangle$ (1)

This also holds for all transformations and is therefore proved instead of the first bisimulation statement. (1) is always shown in the following way:
...
For some transformations the following holds for all closed terms $s, t$:

$\langle s \rangle \longrightarrow_{\langle prg \rangle} \langle t \rangle \implies s \longrightarrow^*_{prg} t$ (2')

(2') implies the second bisimulation statement (2) (by transitivity and by choosing $t$ arbitrarily from the infinite reduction sequence and $s_0 = s$) and is therefore shown in its stead whenever possible. (2') is always shown analogously to (1).
...
\begin{proof}[Proof of (2)] By induction on the structure of the derivation $\mathcal{D}$ of the first step $\langle s \rangle \longrightarrow_{\langle prg \rangle} \langle s_1 \rangle$ of the infinite reduction sequence.
...
\begin{prooftree}
\AxiomC{$\mathcal{D}_{\textrm{PM}}$}
\UnaryInfC{$s =^? q \searrow \sigma$ with $(q, s') \in \textrm{Rules}(\langle prg \rangle)$}
\UnaryInfC{$s \longrightarrow_{\langle prg \rangle} s'[\sigma]$}
\end{prooftree}

with $s'[\sigma] = s_1$; the immediate subterms of $s$ are values; $\mathcal{D}_{\textrm{PM}}$ is a derivation of the pattern matching. This transformation doesn't change input terms, thus $\langle s \rangle = s$, $\langle s_1 \rangle = s_1$.

It will be shown: There exists an equation $r$ for which it holds that $`` q = s' "$ is within $\langle r \rangle^{extract\_outer\_des}$ such that there exist terms $t, s_0$ and reduction sequences from $s_0$ to $t$, from $s$ to $t$, and from $s_0$ to $s$. The first of those uses only $r$ . The other sequences use only the equations within $\langle r \rangle^{extract\_outer\_des}$. Using Lemma 2b for $e = extract\_outer\_des$, this gives us the desired $s \longrightarrow^*_{\langle prg \rangle} t$, $s_0 \longrightarrow^*_{\langle prg \rangle} s$ and $s_0 \longrightarrow^*_{prg} t$, since $elim\_multi\_des = extract\_helpers(extract\_outer\_des)$.

The property above follows from the statement below:

For all equations $r$ and all helpers sets $H$ (possibly empty) for which it holds that $`` q = s' "$ is within $\langle r, H \rangle^{extract\_outer\_des}$ there exist terms $t, s_0$ and reduction sequences from $s_0$ to $t$, from $s$ to $t$, and from $s_0$ to $s$. The first of those uses only $r$ and the equations within $H$. The other sequences use only the equations within $\langle r, H \rangle^{extract\_outer\_des}$.

Let $r = `` q_2 = s'_2 "$. The last statement is proved by induction on the structure of $q_2$.

\begin{itemize}

\item \underline{Case 1}: $q_2$ is hole pattern:

$\langle r, H \rangle^{extract\_outer\_des} = \big\langle r, H \big\rangle$

Since those are the only equations in the result, $r = `` q = s' "$ or $`` q = s' "$ is within $H$. We already know there exists a reduction sequence from $s$ to $s_1$ which only uses $`` q = s' "$. Thus we can simply set $s_0 = s$, $t = s_1$.

\item \underline{Case 2}: $q_2 = q'.des(p_1, ..., p_k)$, $q'$ is hole pattern:

The argument is identical to that in Case 1.

\item \underline{Case 3}: $q_2 = q'.des(p_1, ..., p_k)$, $q'$ is destructor pattern:

$\langle r, H \rangle^{extract\_outer\_des} = \langle \langle r, H \rangle^{extract\_des} \rangle^{extract\_outer\_des}$\\
$= \langle `` q' = hname(\langle q' \rangle^{vars}) ", H \cup \{ \big\langle hsig, heqn \big\rangle \} \rangle^{extract\_outer\_des}$

with $hname, hsig, heqn$ as in the definition of $extract\_des$.
