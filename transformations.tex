...
By the ??? extraction function property, there is an equation $`` q_{r'} = t_{r'} "$ such that the following holds:
...
%%- experimental, meant to be reused in proofs of the above property for concrete extraction functions
...
%    \end{cases}
%\end{equation*}
%On the other hand, if $x$ is contained in $q^0_{r'}$, distinguish two cases for $q^0_{r'}$. If $q^0_{r'} = fun(p'_1, ..., p'_k)$, two cases for $q^0_r$ will be distinguished. If $q^0_r = fun(p_1, ..., p_k)$, write $p_x$ for the pattern $p'_i$ that contains $x$ and define:
%\begin{equation*}
%    \sigma_v[x] =
%    \begin{cases}
%      v, & \text{if}\ p \ \text{contains or is}\ x \ \text{with}\ p_x = p_i[p / y] \ \text{and}\ p[v / x] = (\sigma[y])_v \\
%      (p[\sigma])_v, & \text{if}\ p_x[p / x] = p_i \ \text{and}\ p_x = p'_i
%    \end{cases}
%\end{equation*}
%
%It will now be shown that $s' = q_{r'}[\sigma_v]$. Let $s^0, ..., s^n$ be the immediate subterms of $s = s(s^0, ..., s^n)$. It is $s' = s(s^0_v, ..., s^n_v) = q_{r'}(s^0_v, s^1_v, ..., s^n_v) = q_{r'}(q^0_{r'}[\sigma_v], p^1_{r'}[\sigma_v], ..., p^n_{r'}[\sigma_v]) = q_{r'}[\sigma]$, because $s^0_v = q^0_{r'}[\sigma_v]$ and $s^i_v = p^i_{r'}[\sigma_v]$ for $i = 1, ..., n$.
%
%First, it is shown that $s^i_v = p^i_{r'}[\sigma_v]$. For $i = 1, ..., n$, it is $s^i_v = (p^i_r[\sigma])_v$. If $p^i_{r'}$ is a variable, say $x$, it is $p^i_{r'}[\sigma_v] = x[\sigma_v] = \sigma_v[x] = (\sigma[x])_v = (x[\sigma])_v = (p^i_r[\sigma])_v$. If $p^i_{r'} = con(p_1, ..., p_k)$, it is $p^i_{r'}[\sigma_v] = con(p'_1, ..., p'_k)[\sigma_v] = con(p'_1[\sigma_v], ..., p'_k[\sigma_v]) = con((p_1[\sigma])_v, ..., (p_k[\sigma])_v) = (p^i_r[\sigma])_v$.
%
%Second, it is shown that $s^0_v = q^0_{r'}[\sigma_v]$.

For the $\sigma$ in our case, we thus have a $\sigma_v$ with $q_{r'}[\sigma_v] = q_r[\sigma](s^1_v, ..., s^n_v) = s(s^1_v, ..., s^n_v) = s'$. By the ``Subst" rule, a reduction from $s'$ to $t_{r'}[\sigma_v]$ follows.

By the ??? extraction function property, for the equation $`` q_{r'} = t_{r'} "$ (the same as above) it holds that $t_{r'} = q_1, t_1 = q_2, ..., t_{n-1} = q_n$ for some equations $`` q_i = t_i "$ for $i = 1, ..., n-1$ and an equation $`` q_n = t_r "$. Further it holds that the immediate subterms of $q_i[\sigma_v]$ are values for any substitution $\sigma_v$ that substitutes only with values.

As a consequence, and by using the $\sigma_v$ we have from the above, applying the ``Subst" rule for each of these equations gives us a reduction sequence from $t_{r'}[\sigma_v]$ to $t_r[\sigma_v] \sim t_r[\sigma]$.

%After that, consider the equation $`` q_h = t_h "$ for which $q_r \sim q_h$ and $t_r \sim t_h$ holds. There are two ways in which $q_r \sim q_h$ can hold:
%
%\begin{itemize}
%
%\item \underline{Case 1:} The highest-level structures of $q_h$ and $q_r$ are identical\footnote{I.e., $q_h$ and $q_r$ are applications of the same function or of the same destructor.}, and the immediate subterms of $q_h$ are equivalent to the respective (from left to right) immediate subterms of $q_r$.
%
%Then $s'$ is reduced in one step to a result $t'$ with $t' \sim t$. This step is derived by applying the ``Subst" rule for the equation $`` q_h = t_h "$. For that, it still needs to be shown that $s'$ matches this $q_h$. This is done by showing that $s' = q_h[\sigma_v]$, where $\sigma_v$ is defined by replacing each term which $\sigma$ substitutes with its corresponding value.\footnote{A term can only reduce to one value because $\langle prg \rangle^{extract\_helpers(e)}$ is deterministic. To put it another way, for each term there is exactly one corresponding value.}
%
%From $q_h \sim q_r$ it follows that $q_h[\sigma_v] \sim q_r[\sigma_v]$. Because $\sigma$ substitutes only with what are values in $prg$, each term that $\sigma_v$ substitutes with is equivalent to the term of $\sigma$ it replaces. Thus it is $q_h[\sigma_v] \sim q_r[\sigma_v] \sim q_r[\sigma] = s$. Especially, this means that each immediate subterm of $s$ is equivalent to the respective (from left to right) immediate subterm of $q_h[\sigma_v]$. But, for each immediate subterm of $s$, there is only one \textit{value} that is equivalent to it\footnote{This is because of the intended consequence of the equivalence relation which says that, for closed terms $s,t$ with $s \sim t$, one of them reduces to the other.}, and an $s$ with all immediate subterms reduced to values is already $s'$ by the above construction. Thus $s' = q_h[\sigma_v]$.
%
%\item \underline{Case 2:} Either $q_r[\sigma_v]$ reduces to $q_h[\sigma_v]$, for any $\sigma_v$, or the other way round. Let $\sigma_v$ substitute only with values, and let the terms that $\sigma_v$ substitutes with be equivalent to those of $\sigma$.
%
%Then $s'$ is first reduced to $q_h[\sigma_v]$. Afterwards, $q_h[\sigma_v]$ is reduced in one step to $t' := t_h[\sigma_v]$ with $t' \sim t_r[\sigma_v] \sim t_r[\sigma] = t$. This step is derived by applying the ``Subst" rule for the equation $`` q_h = t_h "$. It still needs to be shown that the reduction from $s'$ to $q_h[\sigma_v]$ is possible.
%
%Because the transformed program is deterministic, $q_r[\sigma_v]$ reduces to $q_h[\sigma_v]$, not the other way round. If $s' = q_r[\sigma_v]$, we thus have the desired reduction.
%
%Now, suppose $s' \neq q_r[\sigma_v]$. As in Case 1 above, it still is $q_r[\sigma_v] \sim q_r[\sigma] = s$, and also $s \sim s'$, and thus $s' \sim q_r[\sigma_v]$. It follows that either $s'$ reduces to $q_r[\sigma_v]$, or the other way round. In the first case, put the reductions from $s'$ to $q_r[\sigma_v]$ and from $q_r[\sigma_v]$ to $q_h[\sigma_v]$ behind one another to obtain the desired reduction. In the second case, $q_r[\sigma_v]$ would reduce both to $s'$ and to $q_h[\sigma_v]$. Since the transformed program is deterministic, it follows that either (a) $s'$ is an intermediate result in the reduction from $q_r[\sigma_v]$ to $q_h[\sigma_v]$, or (b) $q_h[\sigma_v]$ is an intermediate result in the reduction from $q_r[\sigma_v]$ to $s'$. If (a) is true, we have the final part of this sequence as the desired sequence. 
%
%\end{itemize}
...
\underline{Property 6} (momentarily the first part of the ??? property)
