...
\item \textbf{Helper extraction:} For a function $e$ that takes an equation $eqn$ and a set of helper functions $H$ and returns $\langle eqn, H \rangle^e := \big\langle eqn', H' \big\rangle$, with $eqn'$ the changed equation and $H'$ the changed set of helper functions, define $extract\_helpers(e)$ as follows:
...
where
\begin{itemize}
\item $H_{eqns} = \{ heqn \in H \textrm{ with } \big\langle eqn, H \big\rangle = \langle eqn \rangle^{e} ~ | ~ eqn \in eqns \}$
