...
$s \longrightarrow^*_{prg} t \implies \langle s \rangle \longrightarrow^*_{\langle prg \rangle} \langle t \rangle$

and

$\langle s \rangle \longrightarrow^*_{\langle prg \rangle} \langle t \rangle \implies \exists s', t': \langle s' \rangle \longrightarrow^*_{\langle prg \rangle} \langle s \rangle \land \langle t \rangle \longrightarrow^*_{\langle prg \rangle} \langle t' \rangle \land s' \longrightarrow^*_{prg} t'$ (2)
...
For some transformations the following holds:

$\langle s \rangle \longrightarrow_{\langle prg \rangle} \langle t \rangle \implies s \longrightarrow^*_{prg} t$ (2')

(2') implies the second bisimulation statement (2) (by transitivity and by choosing $s' = s, t' = t$) and is therefore shown in its stead whenever possible. (2') is always shown analogously to (1).
...
\begin{proof}[Proof of (2)] By showing

$\langle s \rangle \longrightarrow_{\langle prg \rangle} \langle t \rangle \implies \exists s', t': \langle s' \rangle \longrightarrow^*_{\langle prg \rangle} \langle s \rangle \land \langle t \rangle \longrightarrow^*_{\langle prg \rangle} \langle t' \rangle \land s' \longrightarrow^*_{prg} t'$,

from which (2) follows by transitivity. In turn, this statement is shown by induction on the structure of its derivation $\mathcal{D}$.
...
It will be shown: there exist terms $s'', t'$ and reduction sequences from $s''$ to $t'$, from $s''$ to $s$, and from $t$ to $t'$. The first of those uses only an equation $r$ for which it holds that $`` q = s' "$ is within $\langle r \rangle^{extract\_outer\_des}$. The other sequences use only the equations within $\langle r \rangle^{extract\_outer\_des}$. Using Lemma 2b for $e = extract\_outer\_des$, this gives us the desired $s'' \longrightarrow^*_{\langle prg \rangle} s$, $t \longrightarrow^*_{\langle prg \rangle} t'$ and $s'' \longrightarrow^*_{prg} t'$, since $elim\_multi\_des = extract\_helpers(extract\_outer\_des)$.
...
Since it is the only equation in the result, $r = `` q = s' "$. We already know there exists a reduction sequence from $s$ to $t$ which only uses $`` q = s' "$. Thus we can simply set $s'' = s$, $t' = t$.
...
$\langle r \rangle^{extract\_outer\_des} = \langle \langle r, \emptyset \rangle^{extract\_des} \rangle^{extract\_outer\_des}$\\
$= \langle `` q' = hname(\langle q' \rangle^{vars}) ", \{ \big\langle hsig, heqn \big\rangle \} \rangle^{extract\_outer\_des}$

with $hname, hsig, heqn$ as in the definition of $extract\_des$.

Depending upon $q'$, $`` q = s' "$ is either one of $q' = hname(\langle q' \rangle^{vars})$, $heqn$ or within $\langle `` q' = hname(\langle q' \rangle^{vars}) ", \{ \big\langle hsig, heqn \big\rangle \} \rangle^{extract\_outer\_des}$.

\begin{itemize}

\item Case 3.1: $`` q = s' " = `` q' = hname(\langle q' \rangle^{vars}) "$:

TODO: complete with proof
