...
-- TODO: the second property needs to be changed: instead of such an equation in $H$, there needs to be an equation (either $r'$ or in $H$) such that the ??? property below in the proof holds
...
\begin{equation*}
q_{r_{n+m+1}}[\sigma^{n+m+1}_v] \longrightarrow \widetilde{t_r}[\sigma^{n+m+1}_v] =: \widetilde{t}
\end{equation*}
...
Properties 1-5 are proved by induction on the structure of $q_r$, property 6 separately.

\underline{Properties 1-5}
\begin{itemize}
...
\underline{Property 6}

Set $q_i := q_r[q_{r_1} \leftarrow t_{r_1}]...[q_{r_i} \leftarrow t_{r_i}]$.
There is a $q_0$ and $\sigma^0_v$ with $q_r[\sigma] = q_0[\sigma^0_v]$.
Further, there are equations $r_1, ..., r_n$ in $\langle r \rangle^e$ with $q_{r_1} \leq q_0, q_{r_2} \leq q_1, ..., q_{r_n} \leq q_{n-1}$.
...
\end{equation*}
The induction hypothesis, applied to $q'$, yields $r_1, ..., r_m$ in $\langle r \rangle^e$ with $q_{r_1} \leq q', q_{r_2} \leq q'_1, ..., q_{r_m} \leq q'_{n-1}$, with $q'_i := q'[q_{r_1} \leftarrow t_{r_1}]...[q_{r_i} \leftarrow t_{r_i}]$. It is clear that $q' \leq q_r$ and $q'_i \leq q_i$ for $i \in \{1, ..., m\}$. Choose $n = m+1$ to obtain the property.
...
\item Set $q_i := q_r[q_{r_1} \leftarrow t_{r_1}]...[q_{r_i} \leftarrow t_{r_i}]$. There are equations $r_1, ..., r_n$ in $\langle r \rangle^e$ with $q_{r_1} \leq q_0, q_{r_2} \leq q_1, ..., q_{r_n} \leq q_{n-1}$.
\item There are equations $r_{n+1}, ..., r_{n+m}$ in $\langle r \rangle^e$ with $t_n[\sigma^n_v] = q_{r_{n+1}}[\sigma^{n+1}_v]$, $t_{r_{n+1}}[\sigma^{n+1}_v] = q_{r_{n+2}}$, ..., $t_{r_{n+m}}[\sigma^{n+m}_v] = q_{r_{n+m+1}}$.
\item There is an equation $r_{n+m+1}$ in $\langle r \rangle^e$ and a $\sigma'$ with $t_{r_{n+m}}[\sigma] = q_{r_{n+m+1}}[\sigma']$ for any $\sigma$.
\end{itemize}
...
Proof: By induction on the number $n_c$ of constructor patterns within the copattern that is the left-hand side of the argument of $extract$, the following strengthened statement will be proved:

Let $\langle r \rangle^{extract} = \big\langle r', H \big\rangle$. Property 6 holds, and $r_{n+m+1} = r$ if $H = \emptyset$, and $r_{n+m+1} = heqn$ with $\langle r \rangle^{extract} = \langle r'', \{\big\langle hsig, heqn \big\rangle\} \rangle^{extract}$, otherwise.
