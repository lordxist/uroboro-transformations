...
The equation $`` q = s' "$ can either be contained in that part of $\langle prg \rangle$ that results from the application of $d^{core}$ to the relevant part of $prg$, as specified in the definition of $d''$, or it can be in the other part of $\langle prg \rangle$. As can be seen in the definition of $d''$, this other part is taken over unchanged from $prg$. Thus an equation from this part is present in both $prg$ and $\langle prg \rangle$. For such an equation, $q$ has hole pattern. It can then be easily seen that $\langle s \rangle =^? q \searrow \sigma'$ for a $\sigma'$ with $s'[\sigma'] = \langle t \rangle$ by an argument analogous to that of $`` \Rightarrow "$, ``Subst" case, Case 1.
