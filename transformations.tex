...
\item For any $\sigma^n_v$ that substitutes only with values there exist $\sigma^{n+1}_v, ..., \sigma^{n+m}_v$ that substitute only with values and for which it holds: $q_n[\sigma^n_v] = q_{r_{n+1}}[\sigma^{n+1}_v]$, $t_{r_{n+1}}[\sigma^{n+1}_v] = q_{r_{n+2}}$, ..., $t_{r_{n+m}}[\sigma^{n+m}_v] = q_{r_{n+m+1}}$.
\item For $i \in \{1, ..., m\}$, $q_{r_{n+i}}$ is hole pattern. Further, if $m \neq 0$, $q_{r_{n+m+1}}$ is hole pattern.
\item It is $t_{r_{n+m+1}} = t_r$. Further, for any $\sigma$ that substitutes only with values there exists a $\sigma'$ that substitutes only with values and for which it holds: $t_{r_{n+m}}[\sigma] = q_{r_{n+m+1}}[\sigma']$.
...
It remains to be shown that $\widetilde{t} = t_r[\sigma']$ is equivalent to $t = t_r[\sigma]$. As shown above, the previous reduction steps preserve the equivalence, that is, it is $q_{r_{n+m+1}}[\sigma'] \sim q_r[\sigma]$. Because $q_{r_{n+m+1}} \sim q_r$ by property 6, it follows that $\sigma'$ and $\sigma$ substitute for the same variables, and for each variable, the terms that they substitute are equivalent. From this it immediately follows that $\widetilde{t} = t_r[\sigma'] \sim t_r[\sigma] = t$.
