...
\item Implemented in \texttt{UroboroTransformations.defunc}.
\end{itemize}
...
& && && \{ fun(p_1(x_{1,1}, ..., x_{1,k_1}), ..., p_n(x_{n,1}, ..., x_{n,k_n})) = \\
& && &&\quad \langle fun, `` \textrm{extract} ", prg \rangle^{gensym}(x_{1,1}, ..., x_{n,k_n}) \\
& && && | ~ `` fun(p_1(...), ..., p_n(...)).des(p'_1, ..., p'_k) = t " \in eqns \} \\
& && | && `` \textrm{\textbf{function }} fun(\tau_1, ..., \tau_n): \sigma \textrm{\textbf{ where }} eqns " \textrm{ with mixed } eqns \} \\
&\cup && \{ && \textrm{\textbf{function }} \langle fun, `` \textrm{extract} ", prg \rangle^{gensym} (\tau_1, ..., \tau_n): \sigma \textrm{\textbf{ where }} \\
& && && \{ \langle fun, `` \textrm{extract} ", prg \rangle^{gensym}(x_{1,1}, ..., x_{n,k_n}).des(p'_1, ..., p'_k) = t \\
& && && | ~ `` fun(p_1(x_{1,1},...,x_{1,k_1}), ..., p_n(x_{n,1}, ..., x_{n,k_n})).des(p'_1, ..., p'_k) = t " \in eqns \} \\
& && | && `` \textrm{\textbf{ function }} fun(\tau_1, ..., \tau_n): \sigma \textrm{\textbf{ where }} eqns " \textrm{ with mixed } eqns \} \\
