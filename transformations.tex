...
\item $\langle p \rangle^{vartypes}$ gives the types of the variables appearing in the pattern (from left to right) by going recursively through the pattern's constructor calls.
...
&\cup && \{ && \textrm{\textbf{function }} \langle `` \textrm{extract} ", \langle fun(p_1, ..., p_n) \rangle^{name\_pattern}, prg \rangle^{gensym} (\\
& && &&\quad \langle p_1 \rangle^{vartypes}, ..., \langle p_n \rangle^{vartypes} \\
