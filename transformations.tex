...
It will be shown: there exists a reduction sequence $s \longrightarrow^*_{prg'} t$ for some $prg'$ that contains only the equations within $\langle `` q = s' " \rangle^{extract\_outer\_des}$. Using Lemma 2.1 for $e = extract\_outer\_des$, this gives us the desired $s \longrightarrow_{\langle prg \rangle}^* t$, since $elim\_multi\_des = extract\_helpers(extract\_outer\_des)$.

By the condition of the ``Subst" case, we have a one-step reduction sequence $s \longrightarrow_{prg} t$ that uses only $`` q = s' "$. Therefore, by setting $H = \emptyset$ and since the removal of rules doesn't change a positive value judgement, the existence of the reduction sequence follows from the following statement for arbitrary copatterns $q$, terms $s'$, closed terms $s,t$, and some helpers set $H$, which will be proved by induction on the structure of $q$:

If there exists a reduction sequence $s \longrightarrow^*_{prg'} t$ that in its derivation uses only $`` q = s' "$ and $H$, with $prg'$ a program with these as the only equations, there also exists such a reduction sequence with respect to the equations within $\langle `` q = s' ", H \rangle^{extract\_outer\_des}$.
...
Thus the only resulting equations are the same as the original, for which we already know that the desired reduction sequence exists.
...
$\langle `` q = s' " \rangle^{extract\_outer\_des} = \langle \langle `` q = s' " \rangle^{extract\_des} \rangle^{extract\_outer\_des}$

There is a reduction sequence that uses only $`` q = s' "$ and $H$, let this sequence be:
\begin{equation*}
s_0 \longrightarrow_{prg'} s_1 \longrightarrow_{prg'} ... \longrightarrow_{prg'} s_{n-1} \longrightarrow_{prg'} s_n
\end{equation*}
with $s = s_0, s_n = t$. It is clear that each of these reduction steps can only use $`` q = s' "$ and $H$. By Lemma 3, for each of these steps there is a reduction sequence $s_i \longrightarrow^*_{prg''} s_{i+1}$ that uses only the equations $\langle `` q = s' ", H \rangle^{extract\_des}$, with $prg''$ a program with only these equations. It follows that there is also such a sequence $s \longrightarrow^*_{prg''} t$.

From the definition of $extract\_des$ we have that:

$\langle `` q = s' ", H \rangle^{extract\_des} = \langle `` q'.des(p_1, ..., p_k) = s' " \rangle^{extract\_des} = \big\langle `` q' = t_h ", H' \big\rangle$ for some term $t_h$ and some helpers set $H' \supset H$. 

We thus have a reduction sequence from $s$ to $t$ with respect to the equations within $\big\langle `` q' = t_h ", H' \big\rangle$. By the induction hypothesis, applied to $q'$, we have the desired reduction sequence.
