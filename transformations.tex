...
Well-typedness doesn't hold since types defined by data definitions aren't part of $prg'$. This problem can be solved by temporarily adding ``dummy" empty codata definitions for the missing types, and by adding empty function definitions for (1) each function in $prg$ but not in $prg'$ and (2) each function that is a subsumed constructor. Call this changed program $prg''$.   Now, by (*) we have $\langle s \rangle \longrightarrow_{\langle prg'' \rangle^{d^{core}}} \langle t \rangle$. In the definition of $d^{core}$ it can be seen that empty codata definitions are converted to empty function definitions. A reduction cannot depend upon such an empty function definition. Also, the empty function definitions are converted to empty data definitions. A reduction cannot depend upon those either, as there are no constructors a pattern matching could make a case distinction upon. Thus those definitions can be replaced by the original data definitions for the same types from $prg$ without breaking the reduction from $s$ to $t$. The resulting program is the union of $\langle prg' \rangle^{d^{core}}$ and those original data definitions.

But this program is a subset of $\langle prg \rangle$, as can be seen in the definition of $d''$. (The full $\langle prg \rangle$ is obtained by adding the original function definitions with hole pattern left-hand sides.) Thus we have the desired $\langle s \rangle \longrightarrow_{\langle prg \rangle} \langle t \rangle$.
...
By the induction hypothesis we have a derivation for $\langle s' \rangle \longrightarrow_{\langle prg \rangle} \langle t' \rangle$. Let $\langle \mathcal{E} \rangle$ denote the transformation of $\mathcal{E}$, defined analogously to the transformation of terms by transforming the terms in $\mathcal{E}$ and by setting $\langle [] \rangle = []$. By applying the congruence rule we get $\langle \mathcal{E} \rangle[\langle s' \rangle] \longrightarrow_{\langle prg \rangle} \langle \mathcal{E} \rangle[\langle t' \rangle]$. It is clear that $\langle \mathcal{E} \rangle[\langle s' \rangle] = \langle \mathcal{E}[s'] \rangle = \langle s \rangle$ and $\langle \mathcal{E} \rangle[\langle t' \rangle] = \langle \mathcal{E}[t'] \rangle = \langle t \rangle$.
