...
\AxiomC{...}
\AxiomC{$\vdash_v t_n$}
\RightLabel{V\textsubscript{CodTFun}, if $fun(t_1, ..., t_n)$ has codata type}
\TrinaryInfC{$\vdash_v fun(t_1, ..., t_n)$}
\end{prooftree}
...
\AxiomC{...}
\AxiomC{$\vdash_v t_n$}
\RightLabel{V\textsubscript{CodTDes}, if $t_0.des(t_1, ..., t_n)$ has codata type}
\QuaternaryInfC{$\vdash_v t_0.des(t_1, ..., t_n)$}
\end{prooftree}
...
The case is a bit different with the Codata Fragment, however, as their evaluation context notion for it isn't conservatively extended by this work's. This is because, in their value rule for the Codata Fragment, a term is a value if and only if it is a function application with argument values. In this work, a value can also be a destructor application if it has codata type, but on the other hand, it cannot be a function application if this function doesn't have codata type. 

However, this second difference doesn't exist under the assumption of well-typedness, since, in the Codata Fragment, a type can only be added by a codata definition. Under this assumption, only the first difference remains, but this means that only more reductions are possible. As these are the only differences, this work's reduction notion is still an (albeit not conservative) extension of the Codata Fragment's notion for well-typed programs.
...
with $s'[\sigma] = t$; the immediate subterms of $s$ are values; $\mathcal{D}_{\textrm{PM}}$ is a derivation of the pattern matching. This transformation changes input terms, thus $\langle s \rangle = \langle s \rangle^d$, $\langle t \rangle = \langle t \rangle^d$. $d$ is the defunctionalization of terms defined at the end of section 2.

\begin{itemize}

\item \underline{Case 1}: $q$ is hole pattern:

Then the function definition that contains $`` q = s' "$ contains only equations where the right-hand side is a hole pattern (other cases are excluded by the relevant input fragment for $d''$). Such equations (and indeed the function definitions) are left unchanged by $d''$ except for defunctionalizing the right-hand term, as can be seen directly in the definition of $d''$ (last set in the highest-level union). Thus Rules($\langle prg \rangle$) contains $(q, \langle s' \rangle)$. 
...
For any program $prg'$, when Rules($prg'$) contains only rules of the form $q$ has, no data types, and no constructor calls in right-hand sides, the result of applying $d''$ to $prg'$ is identical to that of applying $d^{core}$ to it. This can be seen by comparing the definitions of $d''$ and $d^{core}$. When only programs $prg'$ as limited as listed above are considered, the first two sets in the highest-level union in the definition of $d''$ are equal to the definition of $d^{core}$. (TODO: not entirely correct, since empty function defs are excluded -- perhaps best to change $d''$) And only these two sets are not empty under these conditions.

Now, consider a program $prg'$ which contains only $q$. Since this is the only rule used in the precondition of the ``Subst" case, it follows that:

$s \longrightarrow_{prg'} t$

This even holds when well-typedness of $prg'$ is dropped, as long as the subterm value conditions of the ``Subst" rule are dropped as well. This is because pattern matching is purely syntactical and doesn't depend upon types, and, without the value conditions, thus neither does the ``Subst" rule. Dropping the value conditions only possibly leads to more reductions, but the reduction given above is in any case still possible.

The dropping of well-typedness allows to drop the data type definitions from $prg'$, as well.

Next, go one step further and subsume the constructor calls under the function calls. Since $prg'$ needn't be well-typed, it is not necessary that all called functions actually appear in the program.

After these steps, $prg'$ has the above described form for which it holds that the application of $d''$ to it is identical to the application of $d^{core}$ to it. Especially, this means that

$\langle s \rangle^{d} \longrightarrow_{\langle prg' \rangle^{d''}} \langle t \rangle^{d} \iff \langle s \rangle^{d^{core}} \longrightarrow_{\langle prg' \rangle^{d^{core}}} \langle t \rangle^{d^{core}} $

Remember, that by (*) it is

$\langle s \rangle^{d^{core}} \longrightarrow_{\langle prg' \rangle^{d^{core}}} \langle t \rangle^{d^{core}} \iff s \longrightarrow_{prg'} t $

and thus, by going backwards through the equivalences,

$\langle s \rangle^d \longrightarrow_{\langle prg' \rangle^{d''}} \langle t \rangle^d$.
