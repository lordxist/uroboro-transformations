...
Let $e$ be an argument of $extract\_helpers$. $e$ is said to satisfy the extraction function properties iff, for any equation $r$, the equations within $\langle r \rangle^e$ are
\begin{equation*}
r_1, ..., r_n, r_{n+1}, ..., r_{n+m}, r_{n+m+1}
\end{equation*}
for $n, m \geq 0$ with properties following below.
\begin{enumerate} 
\item There is a $q_0$ and $\sigma^0_v$ with $q_r[\sigma] = q_0[\sigma^0_v]$.
\item Set $q_i := q_r[q_{r_1} \leftarrow t_{r_1}]...[q_{r_i} \leftarrow t_{r_i}]$. It is $q_{r_1} \leq q_0, q_{r_2} \leq q_1, ..., q_{r_n} \leq q_{n-1}$.
\item It is $q_n[\sigma^n_v] = q_{r_{n+1}}[\sigma^{n+1}_v]$, $t_{r_{n+1}}[\sigma^{n+1}_v] = q_{r_{n+2}}$, ..., $t_{r_{n+m}}[\sigma^{n+m}_v] = q_{r_{n+m+1}}$.
\item For $i \in \{1, ..., m\}$, $q_{r_{n+i}}$ is hole pattern. Further, if $m \neq 0$, $q_{r_{n+m+1}}$ is hole pattern.
\item It is $t_{r_{n+m+1}} = t_r$ and there exists a $\sigma'$ with $t_{r_{n+m}}[\sigma] = q_{r_{n+m+1}}[\sigma']$ for any $\sigma$. Further, if $\sigma$ substitutes only with values, so does $\sigma'$.
\item For $i \in \{1, ..., n+m\}$ it is $q_{r_i} \sim t_{r_i}$. Further, it is $q_{r_{n+m+1}} \sim q_r$.
\end{enumerate}
...
\item First, the prefixes of $s = q_r[\sigma]$ are converted. By property 1 there is a $q_0$ and $\sigma^0_v$ with $q_r[\sigma] = q_0[\sigma^0_v]$. Set $q_i := q_0[q_{r_1} \leftarrow t_{r_1}]...[q_{r_i} \leftarrow t_{r_i}]$.
By property 2, we have equations $r_1, ..., r_n$ in $\langle r \rangle^e$ with $q_{r_1} \leq q_0, q_{r_2} \leq q_1, ..., q_{r_n} \leq q_{n-1}$. 
...
\end{equation*}
for each $i \in \{0, ..., n-1\}$. The initial part is responsible for reducing the immediate subterms of the prefix $q_{r_{i+1}}[\sigma^i_v] \leq q_i[\sigma^i_v]$ to values, resulting in $q_i[\sigma^{i+1}_v]$.

This initial part is constructed first. It suffices to reduce the patterns within $q_{r_{i+1}}[\sigma^i_v]$ from left to right, as will be shown next. The form of $q_{r_{i+1}}$ is $fun(p^0_1, ..., p^0_k).des_1(p^1_1, ..., p^1_{k_1})...des_l(p^l_1, ..., p^l_{k_l})$. By property 2, $q_{r_{i+1}}$ is the smallest prefix of $q_i$ that is a left-hand side in $\langle r \rangle^e$. The other equations that need to be considered are those within $\langle r^* \rangle^e$ for an $r^* \neq r$ of $prg$. By induction on the structure of $q_{r^*}$ it can be shown that, if such an equation is a strict prefix of $q_{r_{i+1}}$, then either $r$ or $r^*$ is a ``dead" rule in $prg$. It follows that there is no left-hand side that a strict prefix of $q_{r_{i+1}}[\sigma^i_v]$ matches. Thus it suffices to reduce the patterns from left to right.

We have such a reduction for each pattern from Lemma 3 with a result equivalent to the start. This is because the subterm that is meant to be reduced is, by precondition, already a value in $prg$. It is clear that, in the result of reducing these subterms, within the subterms, the terms that $\sigma^i_v$ substituted for variables are now replaced with their corresponding values. Thus $\sigma^{i+1}_v$ can be defined by replacing exactly those terms with their corresponding values and keeping the others from $\sigma^i_v$.

The final step is derived by first using the ``Subst" rule for the equation $r_{i+1}$, reducing the prefix $q_{r_{i+1}}[\sigma^{i+1}_v] \leq q_i[\sigma^{i+1}_v]$ to $t_{r_{i+1}}[\sigma^{i+1}_v]$, and then applying the ``Cong" rule to reduce $q_i[\sigma^{i+1}_v]$ to $q_i[q_{r_{i+1}} \leftarrow t_{r_{i+1}}[\sigma^{i+1}_v]] =  q_i[q_{r_{i+1}} \leftarrow t_{r_{i+1}}][\sigma^{i+1}_v]= q_{i+1}[\sigma^{i+1}_v]$. That the ``Cong" rule can be applied is clear since the term reduced in the ``Subst" rule is a prefix of $q_i[\sigma^{i+1}_v]$ and thus there is no term to the left of it. By property 6, the left- and right-hand sides of $r_{i+1}$ are equivalent, thus also the result of the final step to its start term. As a consequence of this and of the equivalences for the previous steps, the entire prefix conversion sequence preserves the equivalence.

\item The second part of the sequence is concerned with the conversion of function calls until the final equation that leads to $\widetilde{t}$ can be used. By property 3, we have equations $r_{n+1}, ..., r_{n+m}$ in $\langle r \rangle^e$ with $q_n[\sigma^n_v] = q_{r_{n+1}}[\sigma^{n+1}_v], t_{r_{n+1}}[\sigma^{n+1}_v] = q_{r_{n+2}}[\sigma^{n+2}_v], ..., t_{r_{n+m}}[\sigma^{n+m}_v] = q_{r_{n+m+1}}[\sigma^{n+m+1}_v]$.\footnote{It can also be the case that $m=0$, then $t_n[\sigma^n_v] = q_{r_{n+1}}[\sigma^{n+1}_v]$ and the sequence constructed below is empty.}
From this we have the sequence
\begin{equation*}
q_n[\sigma^n_v] \longrightarrow t_{r_{n+1}}[\sigma^{n+1}_v] \longrightarrow ... \longrightarrow t_{r_{n+m}}[\sigma^{n+m}_v] = q_{r_{n+m+1}}[\sigma^{n+m+1}_v].
\end{equation*}
Its first step and each of its following steps
\begin{equation*}
t_{r_{n+i}}[\sigma^{n+i}_v] \longrightarrow t_{r_{n+i+1}}[\sigma^{n+i+1}_v]
\end{equation*}
can be derived using the ``Subst" rule for the equation $q_{r_{n+i+1}} = t_{r_{n+i+1}}$. This is possible since, as stated above, by property 6 it is $q_n[\sigma^n_v] = q_{r_{n+1}}[\sigma^{n+1}_v]$ and $t_{r_{n+i}}[\sigma^{n+i}_v] = q_{r_{n+i+1}}[\sigma^{n+i+1}_v]$, respectively, and the immediate subterms of $q_{r_{n+i+1}}[\sigma^{n+i+1}_v]$ are values. This is because, by property 4, $q_{r_{n+i+1}}$ is hole pattern, and $\sigma^{n+i+1}_v$ substitutes only with values.

The function call conversion sequence preserves the equivalence because, by property 6, it is $q_{r_{n+i+1}} \sim t_{r_{n+i+1}}$ for each $i \in \{0, ..., m-1\}$.

\item After this, the final equation that leads to $\widetilde{t}$ can be used, as follows.
\begin{equation*}
t_{r_{n+m+1}}[\sigma^{n+m+1}_v] = q_{r_{n+m+1}}[\sigma'] \longrightarrow t_r[\sigma'] =: \widetilde{t}
\end{equation*}
Such a $\sigma'$ exists by property 5. This step is derived using the ``Subst" rule for the final equation $r_{n+m+1}$. This is possible because either (a) $m = 0$ and then $q_{r_{n+m+1}}[\sigma'] = q_{r_{n+1}}[\sigma^{n+1}_v] = q_n[\sigma^n_v]$, which only has immediate value subterms by construction, or (b) $q_{r_{n+m+1}}$ is hole pattern and $\sigma'$ substitutes only with values.
...
\end{equation*}
It is $fun(t^1_v, ..., t^n_v) \sim t[\sigma]$, because $q \sim t$. This, in turn, can be shown as follows. For some equation $r$ of $prg$, $`` q = t "$ is contained in $\langle r \rangle^e = \big\langle r', H \big\rangle$. Consider the list of equations given in the definition of the extraction function properties. If $r$ is not the last of these equations, then, by property 6, it is $q \sim t$. If, on the other hand, it is the last of these equations, it is $q \sim q_r$, and it will be shown that $s^0$ matches $q_r$, contradicting the fact that $s^0$ is a value in $prg$.

Suppose that $s^0$ ($= fun(t^1_v, ..., t^n_v)$) didn't match $q_r$. That is, $q_r$ has the form $fun_2(...).des_1(...)...des_k(...)$ with either (a) the name $fun_2$ different from $fun$, or (b) $k > 0$, or both.

If at least (a) is true, the function name of $q$ is different from $fun$, as well, because $q \sim q_r$. This is the first intended consequence of the equivalence relations. But then $s^0$ wouldn't match $q$, contrary to the assumption.

If only (b) is true, $fun(t^1_v, ..., t^n_v)$ wouldn't match $q$, either. This is because the number of destructor calls in $q_r$ is mirrored in $q$, \footnote{It is the result of adding the number of actual calls to the number of calls embedded in the function name of $q$.} because $q \sim q_r$. This is the second intended consequence of the equivalence relations. Since $k > 0$, this number is certainly larger than the number mirrored in just the name $fun$.

Use the induction hypothesis to reduce the immediate subterms of $t[\sigma]$ to values. By the same argument as in the previous paragraphs, it can be seen that, if $t[\sigma]$ reduces to something at all, this is equivalent to it.
...
The relevant sequence is constructed by Lemma 2.1. As stated in its proof, all the steps before the last are constructed using Lemma 3. It is also stated there that each step is equivalent to the start term, which can be seen by inspecting the proof of Lemma 3. Going through this proof, one can see that any step is constructed in such a way that it is equivalent to the previous.
...
By induction on the structure of $q_r$; in any case, choose $q_0 := q_r$ and $\sigma^0_v = \sigma$.
...
\end{equation*}
Choose $n = 0$, $m = 0$ and $r_1 = r$.\footnote{Note that the fifth property is vacuously true because there is no $t_{r_{n+m}}$.}
...
\end{multline*}
with $hname$, $hsig$ as in the definition of $extract\_des$. (TODO: What follows is immediately clear when extraction is reformulated (with same meaning) to Writer monad style. See also the idea with the boxed plus in the proofs above for this.) Choose
\begin{equation*}
r_n = `` hname(\langle q' \rangle^{vars}).des(p_1, ..., p_k) = t_r ".
\end{equation*}
The second property of the induction hypothesis, applied to $q'$, yields $r_1, ..., r_{n'}$ in $\langle r \rangle^e$ with $q_{r_1} \leq q', q_{r_2} \leq q'_1, ..., q_{r_{n'}} \leq q'_{n'-1}$, with $q'_i := q'[q_{r_1} \leftarrow t_{r_1}]...[q_{r_i} \leftarrow t_{r_i}]$. It is clear that $q' \leq q_r$ and $q'_i \leq q_i$ for $i \in \{1, ..., m\}$. Choose $n = n'+1$ and $m = 0$ to obtain the properties.
...
Choose $n = 0$, $m = 0$, $r_1 = r$.\footnote{Note again that the fifth property is vacuously true because there is no $t_{r_{n+m}}$.}
...
\begin{enumerate}
\item Choose $q_0 = fun(x_1, ..., x_n).des(x'_1, ..., x'_k)$, $\sigma^0_v(x_i) = p_i[\sigma]$, $\sigma^0_v(x'_i) = p'_i[\sigma]$.
\item Choose $n = 1$ and
\begin{equation*}
r_1 = `` fun(x_1, ..., x_n).des(x'_1, ..., x'_k) = hname(x_1, ..., x_n, x'_1, ..., x'_k) ".
\end{equation*}
\item Choose $m = 0$.
\item Irrelevant because $m = 0$.
\item Choose $\sigma'$ such that $x_i[\sigma] = p_i[\sigma']$, and
\begin{equation*}
r_2 = `` hname(p_1, ..., p_n, p'_1, ..., p'_k) = t_r ".
\end{equation*}
\item The only relevant equation is $r_1$ as chosen above, for which the equivalence holds by definition.
\end{enumerate}
...
By induction on the number $n_c$ of constructor patterns within the copattern that is the left-hand side of the argument of $extract$, the following strengthened statement will be proved:

Let $\langle r \rangle^{extract} = \big\langle r', H \big\rangle$. Property 6 holds, and
\begin{itemize}
\item $r_{n+m+1} = r$ if $H = \emptyset$, and $r_{n+m+1} = heqn$ with $\langle r \rangle^{extract} = \langle r'', \{\big\langle hsig, heqn \big\rangle\} \rangle^{extract}$, otherwise
\item The left-hand side of $r_{n+m+1}$ is hole pattern.
\end{itemize}
...
\end{equation*}
By the induction hypothesis, we have the property for $(q_r)^{-con}$.
\begin{enumerate}
\item Set $\sigma'(x) = p_{con}[\sigma]$ for the variable $x$ replacing the left-most inner- most constructor pattern, and $\sigma'(x) = \sigma(x)$ for other variables $x$. By the induction hypothesis, there is a $q'_0$ and $\sigma'^0_v$ with $(q_r)^{-con}[\sigma'] = q'_0[\sigma'^0_v]$. Choose $q_0 = q'_0$ and $\sigma^0_v = \sigma'^0_v$. It is clear that $q_r[\sigma] = (q_r)^{-con}[\sigma'] = q_0[\sigma^0_v]$.
\item By the induction hypothesis we have $r_1, ..., r_{n'}$ with
\begin{equation*}
q_{r_1} \leq q_0, q_{r_2} \leq q_1, ..., q_{r_{n'}} \leq q_{n-1}.
\end{equation*}
(Remember that $q_0$ was chosen as $q'_0$.) Choose $n = n'$.
\item By the induction hypothesis we have $r'_{n + 1}, ..., r'_{n + m'}$ (since $n = n'$) with
\begin{equation*}
t_n[\sigma^n_v] = q_{r'_{n+1}}[\sigma^{n+1}_v], t_{r'_{n+1}}[\sigma^{n+1}_v] = q_{r'_{n+2}}, ..., t_{r'_{n+m'}}[\sigma^{n+m'}_v] = q_{r'_{n+m'+1}}
\end{equation*}
and an $r'_{n + m' + 1}$ and $\sigma'$ with
\begin{equation*}
t_{r_{n+m'}}[\sigma] = q_{r_{n+m'+1}}[\sigma'].
\end{equation*}
Choose $m = m' + 1$, $r_i = r'_i$ for $i \in \{n+1, ..., n+m\}$, and $\sigma^{n+m}_v = \sigma'$.
\item As chosen, it is $r_i = r'_i$ for $i \in \{n+1, ..., n+m'\}$, and the left-hand sides of these equations are hole patterns by the induction hypothesis (unless $m' = 0$, but then there are no such equations). It also is, as chosen, $r_{n+m} = r'_{n+m'+1}$, which is a hole pattern by the induction hypothesis (of the strengthened statement).
\item Choose $r_{n+m+1} = heqn$. By the induction hypothesis, it is $t_{r_{n+m'+1}} = t_{r_{n+m}} = hname((q_r)^{-con}_{vars^+})$. Further, as chosen, it is $q_{r_{n+m+1}} = hname((q_r)^{-con}_{vars^-})$. Thus, choose $\sigma'$ as follows: Let $x^0$ be the variable at the front of $(q_r)^{-con}_{vars^+}$, and $p^0$ be the pattern at the front of $(q_r)^{-con}_{vars}$. Set $\sigma'(x^0) = p^0[\sigma]$, and $\sigma'(x) = \sigma(x)$ for $x \neq x^0$.
\item First, note again that $m = m'+1$, as chosen above. By the induction hypothesis, it is $q_{r_i} \sim t_{r_i}$ for $i \in \{1, ..., n+m'\}$ by the sixth property, and $t_{r_{n+m'+1}} = hname((q_r)^{-con}_{vars^+}$ by the fifth property. By the first strengthening property of the induction hypothesis, the left-hand side of $r_{n+m'+1}$ is either $hname(((q_r)^{-con})^{-con}_{vars^-})$ or $hname((q_r)^{-con}_{vars^+})$. In both cases it is equivalent to $hname((q_r)^{-con}_{vars^+})$.
\end{enumerate}
...
%with $\mathcal{E}[s^0] = s$ and $\mathcal{E}[t^0] = t$.
%
%First note that Lemma 4 is applicable for $\langle prg \rangle$, because
%\begin{equation*}
%elim\_multi\_des = extract\_helpers(extract\_outer\_des),
%\end{equation*}
%and $extract\_outer\_des$ fulfils the extraction function properties, as shown in the ``Subst" case.
%
%The desired sequence can be constructed as follows.
%
...
%\end{proof}
%
%Note that the ``Cong" case doesn't depend upon the concrete transformation, only on the property that the transformation is $extract\_helpers(e)$ for some helper extraction function $e$ that fulfils the extraction function properties. Thus it can be reused in proofs of (1a) for other transformations such that only the proof for the ``Subst" case needs to be given individually for each proof.
%
%\begin{proof}[Proof of (1b)] By inspecting the proof of (1a).
%
%\begin{enumerate}
%
%\item \textbf{``Subst" case}:
%
%The relevant sequence is constructed by Lemma 2. As stated in its proof, all the steps before the last are constructed using Lemma 4. It is also stated there that each step is equivalent to the start term, which can be seen by inspecting the proof of Lemma 4. Going through this proof, one can see that any step is constructed in such a way that it is equivalent to the previous.
%
%\item \textbf{``Cong" case}:
%
...
%with $s'[\sigma] = t$; the immediate subterms of $s$ are values; $\mathcal{D}_{\textrm{PM}}$ is a derivation of the pattern matching. This transformation doesn't change input terms, thus $\langle s \rangle = s$, $\langle t \rangle = t$.
%
%It will be shown that $extract\_patterns$ fulfils the extraction function properties as given in Lemma 2. Using Lemma 2.1 for $e = extract\_patterns$ and $(q_r, t_r) = (q, s')$, this gives us the desired $s \longrightarrow^*_{\langle prg \rangle} \widetilde{t}$ for a $\widetilde{t} \sim t$, since $disentangle_d = extract\_helpers(extract\_patterns)$.
%
%It is now proved that, for each equation $`` q_r = t_r "$ with $\langle `` q_r = t_r " \rangle^{extract\_patterns} = \big\langle `` q_{r'} = t_{r'} ", H \big\rangle$, the extraction function properties hold.
%
%\begin{itemize}
%
%\item \underline{Case 1}: $q_r$ is hole pattern:
