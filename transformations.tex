...
for some intermediate derivation $\mathcal{D'}$ and a subterm $t^0_j$ of $t_j$. By inversion, we know that $t^0_j = t'^0_j.des(t^{1,1}_j, ..., t^{1,k}_j)$.
...
\UnaryInfC{$t'^0_j \longrightarrow `` hname(\langle q \rangle^{vars}) "[\sigma']$}
\RightLabel{\scriptsize $\longrightarrow$\textsubscript{Des\textsubscript{P}}}
\UnaryInfC{$t^0_j \longrightarrow `` hname(\langle q \rangle^{vars}) "[\sigma'].des(t^{1,1}_j, ..., t^{1,k}_j)$}
\UnaryInfC{$\mathcal{D'}_2$}
\UnaryInfC{$t_j \longrightarrow t'_j$}
\end{prooftree}

where $\mathcal{D'}_2$ is $\mathcal{D'}$ where all the occurences of $s'[\sigma]$ that stem from the reduction of $t^0_j$ to it are replaced by $`` hname(\langle q \rangle^{vars}) "[\sigma'].des(t^{1,1}_j, ..., t^{1,k}_j)$. $t'_j$ is defined respectively.
