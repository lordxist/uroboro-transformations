...
For $elim\_multi\_des$, the equivalence relation is defined as the smallest equivalence relation for which the following holds:

\begin{itemize}

\item $q[\sigma] \sim \langle \textrm{extract}, \langle q \rangle^{name\_pattern}, prg \rangle^{autogen}(\langle q \rangle^{vars})[\sigma]$
...
Especially, this means that, for an equation $`` q = t "$ of $prg$, it is $q \not\sim t$.
...
It remains to be shown that the immediate subterms of $q_r[\sigma]$ are values in $prg$. Suppose that such a subterm $s^0$ reduces to some term in $prg$; this term cannot be equivalent to $s^0$. $s^0$ can either reduce by the ``Cong" rule, or by the ``Subst" rule. If it reduces by the ``Subst" rule, by Lemma 2, $s^0$ reduces to a non-equivalent term in $\langle prg \rangle$, as well. If it reduces by the ``Cong" rule, there is some subterm of $s^0$ that reduces by the ``Subst" rule in $prg$ to a non-equivalent term, and by Lemma 2 it also reduces to some non-equivalent term in $\langle prg \rangle$. As a consequence, $s^0$ reduces to some non-equivalent term in $\langle prg \rangle$, as well. It will be shown that this cannot be true.

If $s^0$ reduced to a non-equivalent term in $\langle prg \rangle$, then there would be a reduction from a subterm of $q[\sigma] \sim q_r[\sigma]$ to a non-equivalent term, as well. It follows that not all immediate subterms of $q[\sigma]$ are values. But it is $q[\sigma] = s$, and thus $s$ wouldn't reduce by the ``Subst" rule for $(q, s')$, contrary to the assumption.
