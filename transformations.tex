...
Strong bisimulation implies the weak kind of bisimulation as given by (1) and (2). It is always shown in its stead whenever possible. The proof of each side of strong bisimulation proceeds analogously to that of (1) (but always with $n = 1$).
...
\begin{proof}[Proof of (2)] By inspecting the proof of (1). In Lemma 3 the relevant part of the construction of the sequence from $\langle s \rangle$ to $\langle t \rangle$ is carried out. A one-step reduction that contains application of the ``Subst" rule is always replaced by two one-step reductions (step 3 in Case 2 of the lemma). The first of those starts at a term containing a call to a helper function which has a fresh name (different from all in $prg$) and therefore doesn't typecheck against $prg$. The second reduction both starts and ends at a term containing a call to this helper functions and therefore doesn't typecheck against $prg$, either.

TODO: Make Lemma 3 fit better with statement (1) (left-hand side: only one-step reduction) and, based on this, clarify this proof.
