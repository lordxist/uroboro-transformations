...
TODO: It still needs to be shown that $s'$ matches this $q_h$. Proposal: The program is deterministic, thus a term can only reduce to one value (if a term has a corresponding value, this is unique), and by the proposed added statement to property 2 we have that each subterm of $q_h[\sigma_v]$ is a value whenever $\sigma_v$ substitutes only with values. We can then find a $\sigma_v$ such that $q_h[\sigma_v] \sim q_r[\sigma_v] \sim q_r[\sigma] = s$. But, for each subterm of $s$, there is only one \textit{value} that is equivalent to it, and an $s$ with all subterms reduced to values is already $s'$ by the above construction. Thus $s' = q_h[\sigma_v]$, which means $s'$ matches $q_h$.
