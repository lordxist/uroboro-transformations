...
TODO: Change this to fit with the new definition of (2) and complete the proof.
...
It will be shown: there exists a reduction sequence from $s$ to $t$ which uses only an equation $r$ for which it holds that $`` q = s' "$ is within $\langle r \rangle^{extract\_outer\_des}$. Using Lemma 2b for $e = extract\_outer\_des$, this gives us the desired $s \longrightarrow^*_{prg} t$, since $elim\_multi\_des = extract\_helpers(extract\_outer\_des)$.
...
Since it is the only equation in the result, $r = `` q = s' "$. We already know there exists a reduction sequence from $s$ to $t$ which only uses $`` q = s' "$.
