...
\item It is $t_n[\sigma^n_v] = q_{r_{n+1}}[\sigma^{n+1}_v]$, $t_{r_{n+1}}[\sigma^{n+1}_v] = q_{r_{n+2}}$, ..., $t_{r_{n+m}}[\sigma^{n+m}_v] = q_{r_{n+m+1}}$.
...
\item First, the prefixes of $s = q_r[\sigma]$ are converted. By property 6 there is a $q_0$ and $\sigma^0_v$ with $q_r[\sigma] = q_0[\sigma^0_v]$. Set $q_i := q_0[q_{r_1} \leftarrow t_{r_1}]...[q_{r_i} \leftarrow t_{r_i}]$.
By property 6, we have equations $r_1, ..., r_n$ in $\langle r \rangle^e$ with $q_{r_1} \leq q_0, q_{r_2} \leq q_1, ..., q_{r_n} \leq q_{n-1}$. 
...
\end{equation*}
can be derived using the ``Subst" rule for the equation $q_{r_{n+i+1}} = t_{r_{n+i+1}}$. This is possible since, as stated above, by property 6 it is $q_n[\sigma^n_v] = q_{r_{n+1}}[\sigma^{n+1}_v]$ and $t_{r_{n+i}}[\sigma^{n+i}_v] = q_{r_{n+i+1}}[\sigma^{n+i+1}_v]$, respectively, and the immediate subterms of $q_{r_{n+i+1}}[\sigma^{n+i+1}_v]$ are values. This is because, by property 6, $q_{r_{n+i+1}}$ is hole pattern, and $\sigma^{n+i+1}_v$ substitutes only with values.
...
\end{equation*}
Such a $\sigma'$ exists by property 6. This step is derived using the ``Subst" rule for the final equation $r_{n+m+1}$. This is possible because either (a) $m = 0$ and then $q_{r_{n+m+1}}[\sigma'] = q_{r_{n+1}}[\sigma^{n+1}_v] = q_n[\sigma^n_v]$, which only has immediate value subterms by construction, or (b) $q_{r_{n+m+1}}$ is hole pattern and $\sigma'$ substitutes only with values.
