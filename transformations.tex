...
$\langle s \rangle \longrightarrow^*_{\langle prg \rangle} \langle t \rangle \implies \exists t': \langle t \rangle \longrightarrow^*_{\langle prg \rangle} \langle t' \rangle \land s \longrightarrow^*_{prg} t'$ (2)
...
(2') implies the second bisimulation statement (2) (by transitivity and by choosing $t' = t$) and is therefore shown in its stead whenever possible. (2') is always shown analogously to (1).
...
It will be shown: there exists a term $t'$ and reduction sequences from $s$ to $t'$ and from $t$ to $t'$, which both use only an equation $r$ for which it holds that $`` q = s' "$ is within $\langle r \rangle^{extract\_outer\_des}$. Using Lemma 2b for $e = extract\_outer\_des$, this gives us the desired $t \longrightarrow^*_{\langle prg \rangle} t'$ and $s \longrightarrow^*_{prg} t'$, since $elim\_multi\_des = extract\_helpers(extract\_outer\_des)$.
...
Since it is the only equation in the result, $r = `` q = s' "$. We already know there exists a reduction sequence from $s$ to $t$ which only uses $`` q = s' "$. Thus we can simply set $t' = t$.
