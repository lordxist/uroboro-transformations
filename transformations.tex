...
\item First, the prefixes of $s = q_r[\sigma]$ are converted. Set $q_i := q_r[q_{r_1} \leftarrow t_{r_1}]...[q_{r_i} \leftarrow t_{r_i}]$.
By property 6, we have equations $r_1, ..., r_n$ in $\langle r \rangle^e$ with $q_{r_1} \leq q_r, q_{r_2} \leq q_1, ..., q_{r_n} \leq q_{n-1}$. 
From this we have the sequence
\begin{equation*}
s = q_r[\sigma^0_v] \longrightarrow^* q_1[\sigma^1_v] \longrightarrow^* ... \longrightarrow^* q_n[\sigma^n_v]
\end{equation*}
with $\sigma^0_v := \sigma$; the other $\sigma^i_v$ are constructed iteratively, as follows below. This sequence consists of the sequences
\begin{equation*}
q_i[\sigma^i_v] \longrightarrow^* q_i[\sigma^{i+1}_v] \longrightarrow q_{i+1}[\sigma^{i+1}_v]
\end{equation*}
for each $i \in \{0, ..., n-1\}$ (it is $q_0 = q_r$). The initial part is responsible for reducing the immediate subterms of the prefix $q_{r_{i+1}}[\sigma^i_v] \leq q_i[\sigma^i_v]$ to values, resulting in $q_i[\sigma^{i+1}_v]$.
...
\item The second part of the sequence is concerned with the conversion of function calls until the final equation that leads to $\widetilde{t}$ can be used. By property 6, we have equations $r_{n+1}, ..., r_{n+m}$ in $\langle r \rangle^e$ with $t_n = q_{r_{n+1}}, t_{r_{n+1}} = q_{r_{n+2}}, ..., t_{r_{n+m}} = q_{r_{n+m+1}}$.\footnote{It can also be the case that $m=0$, then $t_n = q_{r_{n+1}}$ and the sequence constructed below is empty.}
From this we have the sequence
\begin{equation*}
t_n[\sigma^n_v] \longrightarrow^* t_{r_{n+1}}[\sigma^{n+1}_v] \longrightarrow^* ... \longrightarrow^* t_{r_{n+m}}[\sigma^n_v] = q_{r_{n+m+1}}[\sigma^n_v],
\end{equation*}
which consists of the sequences
\begin{equation*}
...
\end{equation*}
...
\begin{equation*}
q_{r_{n+m+1}}[\sigma] \longrightarrow \widetilde{t_r}[\sigma]
\end{equation*}
...
There are equations $r_1, ..., r_n$ in $\langle r \rangle^e$ with $q_{r_1} \leq q_r, q_{r_2} \leq q_1, ..., q_{r_n} \leq q_{n-1}$.
